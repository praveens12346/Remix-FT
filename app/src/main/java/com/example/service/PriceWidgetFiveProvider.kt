            ...
            for (widgetId in appWidgetIds) {
                ...
                for (i in 0 until 5) {
                    if (i < selectedSymbols.size) {
                        ...
                        if (tick != null) {
                            views.setTextViewText(prices[i], tick.price.formatPriceDynamic(info.getDisplayDecimals()))
                            val sign = if (tick.change >= 0) "+" else ""
                            views.setTextViewText(changes[i], "$sign${String.format("%.2f%%", tick.changePercent)}")
                            views.setTextColor(changes[i], if (tick.change >= 0) 0xFF69F0AE.toInt() else 0xFFFF5252.toInt())
                        } else {
                            views.setTextViewText(prices[i], "...")
                            views.setTextViewText(changes[i], "0.0%")
                            views.setTextColor(changes[i], 0xFF9E9E9E.toInt())
                        }
                    } else {
